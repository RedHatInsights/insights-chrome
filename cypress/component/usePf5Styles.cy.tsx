...
// Helper to wait for stylesheet to be present or absent
function waitForStylesheet(shouldExist: boolean, timeout = 5000) {
  cy.document({ timeout }).should((doc) => {
    const link = getPf5Stylesheet(doc);
    if (shouldExist) {
      expect(link, 'PF5 stylesheet should exist').to.exist;
      expect(link?.rel, 'PF5 stylesheet rel attribute').to.equal('stylesheet');
      expect(link?.type, 'PF5 stylesheet type attribute').to.equal('text/css');
      expect(link?.href, 'PF5 stylesheet href').to.include('/apps/chrome/js/pf/pf4-v5.css');
    } else {
