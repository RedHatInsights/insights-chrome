...
* `loosePermissionsKessel` - **(Deprecated)** check Kessel workspace-scoped permissions using logical OR. Takes an array of Kessel **relation** strings (e.g. `rbac_roles_read`, `rbac_groups_read`). Uses the Kessel SDK (`@project-kessel/react-kessel-access-check`) to call `/api/kessel/v1beta2/checkself` (single) or `/checkselfbulk` (multiple) against the user's default workspace. Returns `true` if at least one relation is `ALLOWED_TRUE`, `false` otherwise. Duplicate relations are deduplicated automatically. **Prefer `useChrome().permissions.check()` or `useSelfAccessCheck` from `@project-kessel/react-kessel-access-check` for new code.**
