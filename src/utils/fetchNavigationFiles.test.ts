...
describe('fetchNavigationFiles', () => {
  ...
  describe('feoGenerated mode', () => {
    ...
    it('rejects bundle entries with a null nested route entry', async () => {
      const rawData = [
        { id: 'valid', title: 'Valid Bundle', routes: [{ href: '/ok', title: 'OK' }] },
        { id: 'malformed', title: 'Malformed', routes: [null] },
      ] as unknown as BundleNavigation[];
      ...
      // The entry with a null child must be dropped so normalizeBundle never
      // destructures null and throws.
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('valid');
    });

    it('rejects bundle entries with a primitive nested navItems entry', async () => {
      const rawData = [
        { id: 'valid', title: 'Valid Bundle', navItems: [{ href: '/ok', title: 'OK' }] },
        { id: 'malformed', title: 'Malformed', navItems: [42] },
      ] as unknown as BundleNavigation[];
    ...
    it('rejects bundle entries with a deeply nested malformed child without throwing', async () => {
      const rawData = [
        {
          id: 'malformed',
          title: 'Malformed',
          navItems: [
            {
              title: 'Group',
              navItems: [{ title: 'Sub', routes: [null] }],
            },
          ],
        },
      ] as unknown as BundleNavigation[];
