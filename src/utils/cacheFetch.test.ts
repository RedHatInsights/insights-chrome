...
describe('cacheFetch stale-version cleanup', () => {
  it('rejects array response from guard and skips cache without failing', async () => {
    const arrayGuard = (data: unknown): data is { [key: string]: unknown } => {
      if (Array.isArray(data)) return false;
      return typeof data === 'object' && data !== null;
    };
    const fetcher = jest.fn().mockResolvedValue([{ id: 1 }, { id: 2 }]); // returns array
    const result = await cacheFetch('test-key', fetcher, undefined, arrayGuard);

    expect(result).toEqual({ data: [{ id: 1 }, { id: 2 }], fromCache: false });
    // Verify it warned about unexpected shape
    expect(mockSetItem).not.toHaveBeenCalled(); // should skip cache
  });

  it('does not overwrite valid cached map when live fetch returns array', async () => {
    // Setup: cache has valid object map
    const validCachedMap = {
      app1: { manifestLocation: '/app1/manifest.json' },
      app2: { manifestLocation: '/app2/manifest.json' },
    };
    mockGetItem.mockResolvedValue({ data: validCachedMap, cachedAt: Date.now() });

    const arrayGuard = (data: unknown): data is { [key: string]: unknown } => {
      if (Array.isArray(data)) return false;
      return typeof data === 'object' && data !== null;
    };
    // Fetcher returns array (invalid)
    const fetcher = jest.fn().mockResolvedValue([{ manifestLocation: '/bad/manifest.json' }]);

    const result = await cacheFetch('test-key', fetcher, undefined, arrayGuard);

    // Live array is returned even though it failed validation
    expect(result.data).toEqual([{ manifestLocation: '/bad/manifest.json' }]);
    expect(result.fromCache).toBe(false);
    // Cache was NOT updated with the array
    expect(mockSetItem).not.toHaveBeenCalled();
    // If fetcher had failed, cache would be returned instead
  });
