 ...
 */
export function BuildWorkspaceTree(wps: Workspace[]): TreeViewWorkspaceItem | undefined {
  if (wps.length == 0) {
    return undefined;
  }
...
/**
 * Finds the given workspace in the given tree.
 * @param tree the tree to be traversed.
 * @param workspace the workspace to be found.
 * @returns the tree's item that matches the given workspace, or undefined if
 * it was not found.
 */
export function FindWorkspaceInTree(tree: TreeViewWorkspaceItem, workspace: Workspace): TreeViewWorkspaceItem | undefined {
  // Use a depth first search approach to try to find the workspace in the
  // tree.
  const nodes: TreeViewWorkspaceItem[] = [tree];
  while (nodes.length > 0) {
    const node = nodes.pop();

    if (!node) {
      console.log(`ERROR: unable to find workspace ${workspace} in ${tree} because the current node is undefined`);
      continue;
    }

    // When the current node we are traversing contains the workspace, simply
    // return it and end the loop.
    if (node.workspace.id === workspace.id) {
      return node;
    }

    // When we haven't found the workspace yet, and the node has more children,
    // push them to the list of nodes we have to traverse.
    if (node.children) {
      node.children.forEach((child) => {
        if (instanceOfTreeViewWorkspaceItem(child)) {
          ...
          console.log(
            `ERROR: unable to push node ${node}'s child ${child} to the nodes we need to traverse because it's not a TreeViewWorkspaceItem`
          );
        }
      });
    }
