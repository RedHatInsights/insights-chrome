...
const SearchFeedback = ({ query, results }: SearchFeedbackProps) => {
  ...
  return (
    <MenuGroup className="chr-c-search-feedback pf-v5-u-px-md" label="Are these results helpful?">
      <MenuItem className="pf-v5-u-px-xs" onClick={() => trackFeedback('chrome.search-query-feedback-positive')}>
        <Icon isInline>
          <OutlinedThumbsUpIcon className="pf-v5-u-color-200" />
        </Icon>
      </MenuItem>
      <MenuItem className="pf-v5-u-px-xs" onClick={() => trackFeedback('chrome.search-query-feedback-negative')}>
        <Icon isInline>
