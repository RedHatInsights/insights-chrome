...
describe('onGetAllTags', () => {
    ...
    it('should not fail with no results', () => {
        const { tags: { items } } = onGetAllTags({}, { payload: {} });
        expect(items).toMatchObject([]);
    });

    it('should set all meta data', () => {
        const { tags: { page, perPage, total, count, isLoaded } } = onGetAllTags({}, {
            payload: {
...
describe('onGetAllSIDs', () => {
    ...
    it('should set all meta data', () => {
        const { sid: { page, perPage, total, count, isLoaded } } = onGetAllSIDs({}, {
            payload: {
            // eslint-disable-next-line camelcase
                per_page: 10,
                page: 1,
