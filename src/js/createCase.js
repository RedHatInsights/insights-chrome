...
export function createSupportCase(userInfo, fields) {
    ...
    .then(data => data && window.open(`https://access.${window.insights.chrome.isProd ? '' : 'qa.'}redhat.com/support/cases/${data.session.id}`) && createSupportSentry(data.session.id, fields))
