...
describe('OIDCSecured', () => {
  ...
  it('gracefully handles "Session not active" error with message property (v3.x)', () => {
    const { useAuth } = jest.requireMock('react-oidc-context');
    ...
    // @ts-expect-error: Partial mock of ErrorContext for testing
    mockAuth.error = sessionNotActiveError;
    useAuth.mockReturnValue(mockAuth);
  ...
  it('gracefully handles "Token is not active" error with message property (v3.x)', () => {
    const { useAuth } = jest.requireMock('react-oidc-context');

    const tokenNotActiveError = {
      message: 'Token is not active',
      name: 'Error',
    };

    // @ts-expect-error: Partial mock of ErrorContext for testing
    mockAuth.error = tokenNotActiveError;
    useAuth.mockReturnValue(mockAuth);
  ...
  it('gracefully handles "Token not active" error with message property (v3.x)', () => {
    const { useAuth } = jest.requireMock('react-oidc-context');
  ...
  it('gracefully handles session error with error_description property (v2.x backwards compatibility)', () => {
    const { useAuth } = jest.requireMock('react-oidc-context');
